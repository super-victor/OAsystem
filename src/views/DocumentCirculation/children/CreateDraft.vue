  ...
  export default {
    ...
    methods: {
      ...
      saveDocument(){
        ...
        let obj = {...this.formData,sendfileId:this.sendfileId};
        if(obj.isPublic==='') obj.isPublic = -1;
        sendFileAPI.updateDocument(obj)
        .then(res=>{
          this.$notify({
            title: '保存成功',
        ...
        this.$refs[formName].validate((valid) => {
          if(valid){
            ...
            .then(res=>{
              ...
              this.saveDocumentLoading = false;
              this.canOut = true;
              this.$router.push(`confirm-draft/${this.sendfileId}`);
            })
            .catch(err=>{
              this.$message.error('保存失败');
              this.saveDocumentLoading = false;
    ...
    mounted() {
      ...
      .catch(err=>{
        if(err.toString() != 'Error: 权限认证错误') this.$message.error('新建拟稿失败');
      })
    },
    beforeDestroy() {
